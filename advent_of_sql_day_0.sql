...
------Top cities per country by average naughty-nice score
select  C.country,
		C.city,
		avg(C.naughty_nice_score) as AverageScore,
		count(distinct c.child_id) as ChildCount,
		rank() over (	partition by C.country
						order by avg(C.naughty_nice_score) desc) as ScoreRank
from children C
join christmaslist cl
	on c.child_id = cl.child_id
group by 	C.country,
			C.city 
