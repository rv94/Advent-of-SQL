----Get data
with cte as	(SELECT  sl.*,
					age(timestamp, lag(timestamp) over (order by timestamp)) as timediff,
				    rank() over (	partition by a.place_name
				    				order by timestamp),
			...
			where 1=1)
select 	area_name,
		sum(timediff) as Total_Time_Elapsed
from cte
where 1=1
and rank != 1
group by area_name;
