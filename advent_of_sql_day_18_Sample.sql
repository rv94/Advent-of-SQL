...
SELECT 	*,
		count(manager_id) over (partition by manager_id) as peers_same_manager,
		count(level) over (partition by level) as total_peers_Same_level
from t
order by 	count(level) over (partition by level) desc,
			level,
			staff_id;
